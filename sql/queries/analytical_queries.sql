...
SELECT
    ...
    AVG(fs.line_total) AS avg_revenue_per_customer
FROM warehouse.fact_sales fs
...
SELECT
    ...
    dp.category,
    SUM(fs.profit) AS total_profit,
    ROUND((SUM(fs.profit) / SUM(fs.line_total)) * 100, 2) AS profit_margin,
...
SELECT
    dd.day_name,
    AVG(daily_rev) AS avg_daily_revenue,
    AVG(daily_txn) AS avg_daily_transactions,
    SUM(daily_rev) AS total_revenue
FROM (
    SELECT
        dd.day_name,
        dd.date_key,
        SUM(fs.line_total) AS daily_rev,
        COUNT(DISTINCT fs.transaction_id) AS daily_txn
    FROM warehouse.fact_sales fs
    JOIN warehouse.dim_date dd
      ON fs.date_key = dd.date_key
...
SELECT
    CASE
        ...
        WHEN discount_amount <= line_total * 0.10 THEN '1-10%'
        WHEN discount_amount <= line_total * 0.25 THEN '11-25%'
        WHEN discount_amount <= line_total * 0.50 THEN '26-50%'
        ELSE '50%+'
